...
export function Medications() {
  const { medications, pets, navigate, loadPetData } = useApp();
    ...
    <div className="flex flex-col bg-background min-h-screen pb-20 md:pb-6">
      <div className="bg-card border-b border-border px-5 pt-10 pb-4 md:pt-8">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <h1 className="font-bold text-2xl text-foreground">Medicações</h1>
      ...
      <div className="flex-1 max-w-4xl mx-auto w-full px-5 py-4 flex flex-col gap-3 md:grid md:grid-cols-2 md:items-start">
        {medications.length === 0 ? (
          <div className="col-span-2 flex flex-col items-center justify-center py-16 gap-3 text-center">
            <Pill size={40} className="text-muted-foreground/40" />
            <p className="text-foreground font-medium">Nenhuma medicação cadastrada.</p>
            <p className="text-sm text-muted-foreground">Adicione medicações do seu pet para acompanhar os tratamentos.</p>
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Badge label={m.type} className="text-muted-foreground bg-muted capitalize" />
                    </div>
                    <p className="text-xs text-muted-foreground mt-0.5">{pet?.name} · {m.dosage}</p>
                    <p className="text-xs text-muted-foreground">Frequência: {m.frequency}</p>
                    <p className="text-xs text-muted-foreground">Até: {fmtDate(m.endDate)}</p>
